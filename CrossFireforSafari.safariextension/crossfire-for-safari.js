(function(document) {
    KEY = { DOWN: 40, UP: 38, LEFT: 37, RIGHT: 39 };
    KEY.MODIFIER = 16;

    var xlinks = [];
    var ylinks = [];
    var modifierPressed = false;
    var CROSSFIRE_SAFARI_FOCUS = "crossfire-safari-focus";

    addStyle(".crossfire-safari-focus:focus {outline: 2px solid #6baee6;}");
...
//      var activeRect = {left:-100, right:-100, top:-200, bottom:-100};
      var activeRect = {left:0, right:0, top:-500, bottom:-500};
      if (active.tagName == "A") {
        ignore = true;
    ...
    function focus(node) {
      node.classList.add(CROSSFIRE_SAFARI_FOCUS);
      var listener = node.addEventListener('blur', function(e) {
        node.classList.remove(CROSSFIRE_SAFARI_FOCUS);
